...
function CategoryForm({ initial, onSave, onCancel }: CategoryFormProps) {
  ...
  return (
    <form onSubmit={submit} className="rounded-2xl p-4 flex flex-wrap items-end gap-3" style={{ background: "var(--wt-soft)", border: "2px solid #a78bfa" }}>
      <div>
        <label style={{ fontSize: "0.7rem", fontWeight: 700, color: "var(--wt-muted)", textTransform: "uppercase", letterSpacing: "0.05em" }}>ไอคอน</label>
        <input value={emoji} onChange={e => setEmoji(e.target.value)} maxLength={4} aria-label="อีโมจิ"
          className="block mt-1 rounded-xl outline-none text-center" style={{ ...inputStyle, width: 56, padding: "0.55rem 0", fontSize: "1.1rem" }} />
      </div>
      <div className="flex-1" style={{ minWidth: 160 }}>
        <label style={{ fontSize: "0.7rem", fontWeight: 700, color: "var(--wt-muted)", textTransform: "uppercase", letterSpacing: "0.05em" }}>ชื่อหมวดหมู่ *</label>
        <input value={name} onChange={e => setName(e.target.value)} autoFocus placeholder="เช่น พัฒนาระบบ"
      ...
      <div>
        ...
        <input type="color" value={color} onChange={e => setColor(e.target.value)} aria-label="สีหมวดหมู่"
...
export default function CategoriesSettingsPage() {
  const { categories, addCategory, updateCategory, removeCategory, logEntries } = useData();
  const [adding, setAdding] = useState(false);
  const [editingId, setEditingId] = useState<string | null>(null);

  const usageOf = (name: string) => logEntries.filter(e => e.category === name).length;
    ...
    <div className="mx-auto" style={{ maxWidth: 720 }}>
      <div className="bg-white rounded-2xl p-5" style={{ border: "2px solid var(--wt-border)", boxShadow: "0 4px 16px rgba(124,58,237,0.08)" }}>
        <div className="flex items-center justify-between gap-3 mb-1">
          <div>
            <h2 style={{ fontSize: "1.05rem", fontWeight: 800, color: "var(--wt-text)" }}>หมวดหมู่</h2>
            <p style={{ fontSize: "0.78rem", color: "var(--wt-muted)" }}>จัดการหมวดหมู่ที่ใช้ตอนบันทึกงานรายวัน</p>
          </div>
          {!adding && (
        ...
        <div className="space-y-2 mt-4">
          ...
          {categories.length === 0 && !adding && (
            <div className="text-center py-10" style={{ color: "var(--wt-muted)" }}>
              <p style={{ fontSize: "0.9rem", fontWeight: 700, color: "var(--wt-text)" }}>ยังไม่มีหมวดหมู่</p>
              <p style={{ fontSize: "0.8rem", marginTop: 2 }}>กด “เพิ่มหมวดหมู่” เพื่อเริ่ม</p>
            </div>
          )}

          {categories.map(cat => (
            editingId === cat.id ? (
              <CategoryForm key={cat.id} initial={cat}
                onSave={data => { updateCategory(cat.id, data); setEditingId(null); }}
              ...
              <div key={cat.id} className="group flex items-center gap-3 rounded-2xl p-3"
                ...
                <div className="flex items-center gap-1 opacity-60 group-hover:opacity-100 focus-within:opacity-100 transition-opacity">
                  ...
                  <button onClick={() => removeCategory(cat.id)} aria-label={`ลบ ${cat.name}`}
        ...
        <p className="mt-4" style={{ fontSize: "0.72rem", color: "var(--wt-muted)" }}>
          <X size={11} className="inline" style={{ verticalAlign: "-1px" }} /> การลบหมวดหมู่ไม่ลบรายการบันทึกที่ใช้หมวดนั้น (รายการเดิมยังเก็บชื่อหมวดไว้) และกดเลิกทำได้
        </p>
      </div>
    </div>
  );
